...
namespace GameLauncher
{
    public sealed partial class MainScreen : Form
    {
        ...
        private void mainScreen_Load(object sender, EventArgs e)
        {
            ...
            _NFSW_Installation_Source = _settingFile.KeyExists("CDN") ? _settingFile.Read("CDN") : "http://download.soapboxraceworld.ml";
            ...
            try {
                ...
                MessageBox.Show(null, "Manipulation detected! Click ok to Enter Danger Zone!", "GameLauncher", MessageBoxButtons.OK);
                //BlacklistedServers.dat Gets Fu*ked Really Annoying :/ -DavidCarbon
                //Process.GetProcessById(Process.GetCurrentProcess().Id).Kill();
            }
            ...
            else
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(extraNode.InnerText))
                    {
                        ...
                        {
                            if (extraNode.InnerText == "I see, you want to be cultured")
                            {
        ...
        {
            Process.Start("https://github.com/DavidCarbon-SBRW/GameLauncher_NFSW-translations/tree/master/Languages");
        }
            ...
            try
            {
                if (WebClientWithTimeout.createHash(_settingFile.Read("InstallationDirectory") + "/nfsw.exe") == "7C0D6EE08EB1EDA67D5E5087DDA3762182CDE4AC")
                {
                    ...
                    else if (!DetectLinux.NativeLinuxDetected())
                    {
                        ...
                        //Removed Game Launcher Updater (So it doesn't break something)
                        if (Environment.OSVersion.Version.Major >= 6)
                        {
                            //ContextMenu.MenuItems.Add(new MenuItem(Language.getLangString("CONTEXT_CHECKUPDATE", _uiLanguage), Updater.CheckForUpdate));
                        }
